...
def run_ferre(lines, stall_after, max_t_communicate, max_t_communicate_first_result):
    """
    Drive ferre() against a faked subprocess; returns (watchdog_fired, elapsed_seconds).

    Note this keys off the watchdog's own log line rather than whether the process object saw a
    kill(): ferre() calls kill() unconditionally as cleanup once the reader loop ends, so a clean
    run and a watchdog kill are indistinguishable from the process object alone.
    """
    ...
    watchdog_fired = any("hanging no communication" in m for m in logged)
    assert not any("MONITOR DIED" in m for m in logged), "the monitor thread raised"
    return watchdog_fired, _time() - started
...
def test_silence_before_first_result_is_not_killed_at_the_steady_state_budget():
    ...
    watchdog_fired, elapsed = run_ferre(
...
def test_silence_before_first_result_is_killed_once_its_own_budget_expires():
    ...
    watchdog_fired, _ = run_ferre(
        lines, stall_after=3, max_t_communicate=60, max_t_communicate_first_result=2
    )
    assert watchdog_fired, "a stall exceeding max_t_communicate_first_result must still be caught"
